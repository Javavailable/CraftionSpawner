...
- **Transactional pending commits:** pending batches now use internal non-destructive claim/ack semantics. Claims and acknowledgements hold the pending lock only briefly; router invocation and location/data/inventory lock acquisition never happen while holding it. `COMMITTED` acknowledges only exact claimed batch IDs. `NOOP`, `LOCK_UNAVAILABLE`, and unexpected exceptions release the claim without dropping newer pending work. `ABORTED_STALE` clears normal pre-generated loot, pending batches, and claim state for the stale instance.
