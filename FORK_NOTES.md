...
- **Commit boundaries:** routing is computed before XP/item/timer mutation. XP uses a data-only `setSpawnerExpData` commit with the same clamp semantics as the public setter; hologram, GUI cache, and viewer updates are guarded post-acknowledgement notifications. Internal item commits use a source-of-truth virtual-inventory insertion helper that precomputes signatures before mutation and defers sell-value/cache refresh to guarded post-commit work. The transaction marks XP and inventory points of no return through explicit callbacks immediately after the durable mutation.
- **Claimless pre-generated recovery:** commit callers explicitly mark whether a generated batch is owned/retryable. Already-cleared pre-generated batches with no pending claim are requeued on genuine pre-side-effect failure or `NOOP` from full storage/full XP/no router attempt, while normal disposable generation cycles continue to skip the cycle intentionally.
